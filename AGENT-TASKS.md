...
8. **引擎里有的就用引擎的，确实没有再自己写**。自己写的每一行局部避让、寻路、碰撞、排序、插值，都是在重新实现一遍别人调了十年的东西，并且会把它早就踩过的坑**重新踩一遍**。这条是有代价换来的：手写的集群避让在人群里死锁到零速、两只恐龙 25 秒内换了 47 次边、直线检测按距离采样跳过了整整一格——**这三个都不是有意思的 bug，是不用 `NavigationAgent3D` 的学费**。
   - 判断标准不是"引擎的那个够不够完美"，是"**我要写的这个，引擎里有没有一个能干同样事的**"。有就用，不合适就**说清楚哪里不合适**再自己写，并把理由写进注释。
   - 一样适用于：`AnimationTree`（而不是自己写状态机）、`Tween`（而不是自己插值）、`NavigationServer3D` 的烘焙（而不是自己算什么地方过得去）、`Area3D` 的信号（而不是每帧遍历找邻居）。
...
## 场景精修：见 [SCENE-POLISH.md](SCENE-POLISH.md) ← **当前主计划**

把世界从"Godot 画出来的几何体"做成成品游戏场景：换真资产、接动画、材质、天空、风、特效。18 条任务。**先派 S1（资产选型）和 S2（动画状态机）**，它们是其余全部任务的输入。

2D 界面的精修计划在 [UI-POLISH.md](UI-POLISH.md)，**已延后**。
...
| # | 任务 | 状态 | 分支 |
|---|---|---|---|
| 1 | 场景环境：`WorldEnvironment` | **已完成**（`08fc285` + review 修复 `9a5228c`） | `v0.5-environment` |
| 2 | 丘陵看起来像丘陵 | **已完成**（`1b6fec0`，范围扩大成了整个山谷地形） | `v0.5-environment` |
| 3 | 资源点的"采空"状态 | **可以派** | `v0.5-resource-states` |

> 派活之前先看这张表的状态列。第 1 条做完之后表没改，差一点被重复派出去一次——**一张过期的交接单比没有交接单更危险**。
